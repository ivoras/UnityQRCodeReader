...
namespace QRCodeDecoderLibrary
	...
	internal class QRCodeFinder
		...
		internal int Col1;
		internal int Col2;
		internal float HModule;

		// vertical scan
		internal int Col;
		internal int Row1;
		internal int Row2;
		internal float VModule;

		internal float Distance;
		internal float ModuleSize;
		...
		internal void Match
			...
			if(Mathf.Min(HModule, VModule) < Mathf.Max(HModule, VModule) * QRDecoder.MODULE_SIZE_DEVIATION) return;

			// calculate distance
			float DeltaX = Col - 0.5f * (Col1 + Col2);
			float DeltaY = Row - 0.5f * (Row1 + Row2);
			...
			if(Delta < Distance)
				...
				ModuleSize = 0.5f * (HModule + VModule);
				Distance = Delta;
				}
			return;
			}
